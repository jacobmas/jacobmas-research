...
\section{Knapsack Version Attempt}
\label{sec:knapsack}

Begin with receiving 


\[\matH = \begin{bmatrix}\matH^{(1)}&\matH^{(2)}\end{bmatrix} \gets R_q^{(k-1) \times 2(k-1)}, \vecb = \matH\vecx \in R_q^{k-1}\]


where $\vecx = (\vecx^{(1)}, \vecx^{(2)}) \gets \chi^{2(k-1)}$, as well as a vector $\vecz \in \calZ \subseteq R_q^{2k}$
